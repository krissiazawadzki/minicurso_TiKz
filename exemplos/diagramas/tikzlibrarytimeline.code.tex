...
\newcommand{\timeline}[2][]{
  ...
  \draw[fill,opacity=0.75] (0,0) rectangle (\timelinewidth,\timelineheight);
  \shade[top color=black, bottom color=white,middle color=black!20, opacity=0.7]
  ...
  \pgfmathsetmacro\position{1.25*\timelinewidth/(\maxsmitem+1)}
  \node at (0,0.5\timelineheight)(\timespan-0){\phantom{Week 0}};
 
  \ifcustominterval%
    \foreach \x[count=\xi] in {#2}{%
      \node[text=white,text depth=0pt, font=\large, opacity=1.0]at +(\xi*\position-0.625*\position,0.5\timelineheight) (\timespan-\xi) {\timespan\ \x};%
    }%
  \else%
